$(document).ready(function () {
            ...
            if (position != -1) {
                $.ajax({
                    ...
                    $.ajax({
                        ...
                        $(".modal-trade").html("<h4>Will you send me [" + food + "]<br>as requested [" + resp.lunch + "]?</h4>");
                        $("#modal2").modal("open");
                    }).catch(function (err) {
                        throw err;
                    });
            ...
            .then(function (resp) {
                ...
                if (resp.length === 0) {
                    ...
                    for (var i = 0; i < resp.length; i++) {
                        ...
                        "<span style='font-size:14px; font-weight:100; font-style: italic;'> (" + resp[i].school + ")</span>"
                       
                        if (resp[i].tradable) {
                                food += " <span style='font-size:14px; font-weight:100; font-style: italic;'>(tradable)</span>";
                            }
                          if(resp[i].requested ) {
                                food += " <span style='font-size:14px; font-weight:100; font-style: italic;'>(requested)</span>";
                            };
    ...
    function tradableLunch() {
        $.ajax({
            ...
            .then(function (resp) {
                ...
                $("#searchItems").empty();
                for (var i = 0; i < resp.length; i++) {
                    if (payload.userID != resp[i].userID && resp[i].tradable||resp[i].requested) {
                        counter++;
                        var food = resp[i].lunch + "<span style='font-size:14px; font-weight:100; font-style: italic;'>("+resp[i].ingredients+")</span>" +
                        // var ingredients = ;
                        "<span style='font-size:14px; font-weight:100; font-style: italic;'> (" + resp[i].createdAt + ")</span>"+
                        "<span style='font-size:14px; font-weight:100; font-style: italic;'> (" + resp[i].eater + ")</span>"+
                        "<span style='font-size:14px; font-weight:100; font-style: italic;'> (" + resp[i].school + ")</span>"
                        if (resp[i].tradable ||resp[i].requested ) {
                                food += " <span style='font-size:14px; font-weight:100; font-style: italic;'>(tradable)</span>";
                            };
                        var foodName = $("<div class='myFoodName col s10 amber lighten-4'>").attr("id", "food" + resp[i].id).html(food);
    ...
    function requestedLunch() {
        $.ajax({
            ...
            .then(function (resp) {
                var counter = 0;
                $("#requestedItems").empty();
                for (var i = 0; i < resp.length; i++) {
                    if (payload.userID != resp[i].userID && resp[i].requested) {
                        counter++;
                        var food = resp[i].lunch + "<span style='font-size:14px; font-weight:100; font-style: italic;'>("+resp[i].ingredients+")</span>" +
                        // var ingredients = ;
                        "<span style='font-size:14px; font-weight:100; font-style: italic;'> (" + resp[i].createdAt + ")</span>"+
                        ...
                        var button = $('<button class="foodTrade waves-effect waves-light btn orange lighten-1">').attr({ "id": resp[i].id, "data-name": resp[i].userID }).text("Send");
                        var foodOption = $("<div class='foodOption col s3'>").append(button);
                        $("#requestedItems").append(foodName).append(foodOption);
                        

                    };
                };
                if (counter === 0) {
                    $("#requestedItems").html("<h4>Nothing...</h4>");
                };
            }).catch(function (err) {
                throw err;
            });
    ...
    $("#allrequested1").on("click", function (event) {
        event.preventDefault();
        requestedLunch();
    });
    ...
    $("#allrequested2").on("click", function (event) {
        event.preventDefault();
        $('.sidenav').sidenav("close");
       requestedLunch();
    });
    ...
    $(document).on("click", ".foodsend", function (event) {
        event.preventDefault();
        ...
        var counter = 0;
        for (var i = 0; i < lunchList.length; i++) {
            if (lunchList[i].requested) {
                counter++;
                var foodName = $("<div class='myFoodName col s10 amber lighten-4'>").attr({ "id": "food" + lunchList[i].id, "value": lunchList[i].id }).html(lunchList[i].lunch);
                var button = $('<button class="mysend waves-effect waves-light btn orange lighten-1">').attr("id", lunchList[i].id).html('<i class="material-icons prefix">autorenew</i>');
                var foodOption = $("<div class='foodOption col s2'>").append(button);
                $("#myItems").append(foodName).append(foodOption);
            }
        };
        if (counter === 0) {
            $("#myItems").html("<h4>Nothing to send...</h4>");
            window.setTimeout(function () {
                window.location.assign("/search.html")
            }, 800);
        };
    ...
    $(document).on("click", ".myTrade", function (event) {
        ...
        $(".modal-content").html("<h4>A REQUEST HAS BEEN SENT(Check Ingredients For Allergens)!</h4>")
        $("#modal1").modal("open");
    ...
    $(document).on("click", ".mysend", function (event) {
        event.preventDefault();

        myFood = Number($(this).attr("id"));
        $("#food" + $(this).attr("id")).addClass("darken-2");

        $(".modal-content").html("<h4>A FOOD REQUEST HAS BEEN SENT(Check Ingredients For Allergens)!</h4>")
        $("#modal1").modal("open");
    ...
    $(document).on("click", "#phonenumber", function (event) {
        event.preventDefault();
        $(".modal-content").html("<h4>Text 702-781-0949 about your request!</h4>")
        $("#modal1").modal("open");
    })
    $(document).on("click", "#agree", function (event) {
        ...
        $.ajax({
            ...
            $.ajax({
                ...
                $(".modal-content").html("<h4>A TRADE HAS BEEN COMPLETED(Check Ingredients For Allergens)!</h4>")
                $("#modal1").modal("open");
